...
export default function ManagerEndOfDayPage() {
  ...
  const fetchSummary = async () => {
    try {
      const response = await fetch("/api/reports/eod/summary");
      if (response.ok) {
  ...
  const handleSubmit = async () => {
    if (!summary || selectedWorkers.length === 0) return;
          ...
          <Card className="border-0 shadow-sm">
            <CardContent className="py-12 text-center">
              ...
              <p className="mt-2 text-sm text-muted-foreground">
                Today&apos;s end of day report has already been created.
              </p>
              <Button 
                className="mt-4" 
  ...
  return (
    <PageContent>
      <div className="space-y-6">
        <PageHeader 
          title="End of Day Report" 
          description={`Create report for ${new Date(summary.date).toLocaleDateString("en-US", { 
            weekday: "long", 
            year: "numeric", 
            month: "long", 
            day: "numeric" 
        ...
        <Card className="border-0 shadow-sm">
          <CardHeader>
            ...
            <CardDescription>
              Select which team members worked today. Workers with logged activity are pre-selected.
            </CardDescription>
          </CardHeader>
          <CardContent>
            ...
            {summary.usageByWorker.length === 0 && (
              <p className="text-center text-muted-foreground py-4">
                No team members found. Make sure you are assigned to a team.
              </p>
            )}
          </CardContent>
        </Card>
        ...
        {selectedWorkers.length > 0 && (
          <Card className="border-0 shadow-sm">
            <CardHeader>
              ...
              <CardDescription>
                Activity logged by selected workers
              </CardDescription>
            </CardHeader>
            <CardContent>
